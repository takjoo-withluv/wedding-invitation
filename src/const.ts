import dayjs from "dayjs"
import utc from "dayjs/plugin/utc"
import timezone from "dayjs/plugin/timezone"
import "dayjs/locale/ko"

dayjs.extend(utc)
dayjs.extend(timezone)
dayjs.locale("ko")

export { dayjs }

export const WEDDING_DATE = dayjs.tz("2026-07-05 13:00", "Asia/Seoul")
export const WEDDING_DATE_FORMAT = `YYYY년 MMMM D일 dddd A h시${WEDDING_DATE.minute() === 0 ? "" : " m분"}`

// 예식 당월 휴무일. 켈린더에 표시하기 위함.
// 예: 예식일 8월 -> 8월 15일 광복절
export const HOLIDAYS = [17]

export const LOCATION = "부산시 동구 범일동 830-140 동일타워 3층"
export const LOCATION_ADDRESS = "부산시 동구 조방로 14"
export const LOCATION_HALL = "디엘웨딩홀 3층 마이아홀"

// 카카오톡 공유 시 위치 정보로 사용할 주소.
// LOCATION 과 동일하게 설정해도 무방하나, 필요에 따라 좀 더 상세히 작성 가능.
export const SHARE_ADDRESS = LOCATION_HALL
export const SHARE_ADDRESS_TITLE = LOCATION_HALL

// 네이버 지도 및 카카오 네비게이션에 사용할 좌표. [경도, 위도] 형식.
export const WEDDING_HALL_POSITION = [129.06345343296, 35.1387281351032]

// 네이버 지도의 웨딩홀 장소 ID
// 네이버 지도 웹페이지에서 웨딩홀 검색 후 URL에서 확인 가능.
// 예: https://map.naver.com/p/entry/place/13321741 -> 13321741
export const NMAP_PLACE_ID = 35413559

// 카카오 지도의 웨딩홀 장소 ID
// 카카오 지도 웹페이지에서 웨딩홀 검색 후 해당 장소에서 상세보기 클릭 시 URL에서 확인 가능.
// 예: https://place.map.kakao.com/8634826 -> 8634826
export const KMAP_PLACE_ID = 25431142

export const BRIDE_FULLNAME = "정은주"
export const BRIDE_FIRSTNAME = "은주"
export const BRIDE_TITLE = "장녀"
export const BRIDE_FATHER = "정덕근"
export const BRIDE_MOTHER = "임경숙"
export const BRIDE_INFO = [
  {
    relation: "신부",
    name: BRIDE_FULLNAME,
    phone: "010-5899-9919",
    account: "국민은행 103002-04-296193",
  },
  /*
  {
    relation: "신부 아버지",
    name: BRIDE_FATHER,
    phone: "010-0000-0000",
    account: "하나은행 00000000000",
  },
  {
    relation: "신부 어머니",
    name: BRIDE_MOTHER,
    phone: "010-0000-0000",
    account: "하나은행 00000000000000",
  },
  */
]

export const GROOM_FULLNAME = "김탁현"
export const GROOM_FIRSTNAME = "탁현"
export const GROOM_TITLE = "장남"
export const GROOM_FATHER = "김양호"
export const GROOM_MOTHER = "박다숙"
export const GROOM_INFO = [
  {
    relation: "신랑",
    name: GROOM_FULLNAME,
    phone: "010-3109-3234",
    account: "국민은행 960102-01-376589",
  },
  /*
  {
    relation: "신랑 아버지",
    name: GROOM_FATHER,
    phone: "010-9327-6755",
    account: "국민은행 117-01-0320-080",
  },
  {
    relation: "신랑 어머니",
    name: GROOM_MOTHER,
    phone: "010-2529-6754",
    account: "부산은행 273-12-010972-6",
  },
  */
]